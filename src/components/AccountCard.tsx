...
const AccountCard = ({ account }: { account: Account }) => {
    return (
        <div className='w-lg h-64 border-2 border-gray-300 rounded-2xl p-8 relative flex flex-col'>
            <p className='text-lg text-gray-800'>card number : {account._id}</p>
            <p className='text-lg text-gray-800'>client number : {account.clientId}</p>
            <p className={`px-2 py-1 rounded ${account.type == 'credit'? 'bg-gray-800' : 'bg-neutral-700'} text-white text-lg absolute top-2 right-2`}>{account.type}</p>
